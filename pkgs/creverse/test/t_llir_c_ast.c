...
TEST(llir_c_ast_api_null_safety)
{
	START;

	llir_c_ast_t ast = {.cflow = (const llir_cflow_t *)1, .ssa = (const llir_ssa_t *)1, .expr = (const llir_expr_t *)1, .vars = (const llir_vars_t *)1,
			    .types = (const llir_types_t *)1};
	EXPECT_EQ(llir_c_ast_init(NULL), NULL);
	EXPECT_EQ(llir_c_ast_gen(NULL, NULL, NULL, NULL, NULL, NULL), 1);
	EXPECT_EQ(llir_c_ast_emit(NULL, DST_NONE()), 0);
	EXPECT_EQ(llir_c_ast_print(NULL, NULL, NULL, NULL, NULL, DST_NONE()), 0);
	llir_c_ast_free(NULL);

	EXPECT_NE(llir_c_ast_init(&ast), NULL);
	EXPECT_EQ(ast.cflow, NULL);
	EXPECT_EQ(ast.ssa, NULL);
	EXPECT_EQ(ast.expr, NULL);
	EXPECT_EQ(ast.vars, NULL);
	EXPECT_EQ(ast.types, NULL);
	...
	EXPECT_EQ(llir_c_ast_gen(&ast, &cflow, &ssa, &expr, &vars, &types), 0);
	EXPECT_EQ(ast.cflow, &cflow);
	EXPECT_EQ(ast.ssa, &ssa);
	EXPECT_EQ(ast.expr, &expr);
	EXPECT_EQ(ast.vars, &vars);
	EXPECT_EQ(ast.types, &types);

	char out[32] = {0};
	EXPECT_GT(llir_c_ast_emit(&ast, DST_BUF(out)), 0);
...
TEST(llir_c_ast_print_api_null_safety)
{
	START;

	char out[32] = {0};
	EXPECT_EQ(llir_c_ast_print(NULL, NULL, NULL, NULL, NULL, DST_BUF(out)), 0);

	END;
}

TEST(llir_c_ast_print_no_vars)
{
	START;
	...
	size_t len = llir_c_ast_print(&cflow, &ssa, &expr, &vars, &types, dst);
	EXPECT_GT(len, 0);
...
TEST(llir_c_ast_print_type_fallback)
{
	START;
	...
	char out[256] = {0};
	dst_t dst = DST_BUF(out);
	size_t len = llir_c_ast_print(&cflow, &ssa, &expr, &vars, NULL, dst);
	EXPECT_GT(len, 0);
...
TEST(llir_c_ast_print_type_lookup_miss)
{
	START;
...
TEST(llir_c_ast_print_exhaustive)
{
	START;
...
TEST(llir_c_ast_print_control_flow_paths)
{
	START;
	...
	char out[512] = {0};
	dst_t dst = DST_BUF(out);
	size_t len = llir_c_ast_print(&cflow, &ssa, &expr, NULL, NULL, dst);
	EXPECT_GT(len, 0);
	EXPECT_EQ(t_c_str_contains(STRVN(out, len), STRV("void recovered(void)")), 1);
...
TEST(llir_c_ast_print_fallback_nodes)
{
	START;
...
TEST(llir_c_ast_print_unknown_unary)
{
	START;
...
TEST(llir_c_ast_print_unknown_binary)
{
	START;
...
TEST(llir_c_ast_print_phi_without_args)
{
	START;
...
TEST(llir_c_ast_print_terminal_if)
{
	START;
...
TEST(llir_c_ast_print_loop_structuring)
{
	START;
...
TEST(llir_c_ast_print_if_else_structuring)
{
	START;
...
STEST(llir_c_ast)
{
	...
	RUN(llir_c_ast_api_null_safety);
	RUN(llir_c_ast_print_api_null_safety);
	RUN(llir_c_ast_print_no_vars);
	RUN(llir_c_ast_print_type_fallback);
	RUN(llir_c_ast_print_type_lookup_miss);
	RUN(llir_c_ast_print_exhaustive);
	RUN(llir_c_ast_print_control_flow_paths);
	RUN(llir_c_ast_print_fallback_nodes);
	RUN(llir_c_ast_print_unknown_unary);
	RUN(llir_c_ast_print_unknown_binary);
	RUN(llir_c_ast_print_phi_without_args);
	RUN(llir_c_ast_print_terminal_if);
	RUN(llir_c_ast_print_loop_structuring);
	RUN(llir_c_ast_print_if_else_structuring);
