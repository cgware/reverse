...
#define ELFC_BIN(elfc) (&(bin_t){.buf = (elfc)->bytes})
...
int elfc_parse_ident(elfc_t *elfc, size_t *off, elfc_sect_t *sect, alloc_t alloc)
{
	static const schema_val_t classes[] = {
		{ELFC_IDENT_CLASS_32, STRVT("32-bit format")},
		{ELFC_IDENT_CLASS_64, STRVT("64-bit format")},

	};

	static const schema_val_t datas[] = {
		{ELFC_IDENT_DATA_LE, STRVT("Little endian")},
		{ELFC_IDENT_DATA_BE, STRVT("Big endian")},
	...
	read_layout(ELFC_BIN(elfc), off, &sect->data.elf_ident.schema, sect->data.elf_ident.layout, sect->data.elf_ident.data);
...
int elfc_parse_header(elfc_t *elfc, size_t *off, u8 class, elfc_sect_t *sect, alloc_t alloc)
{
	...
	switch (class) {
	case ELFC_IDENT_CLASS_32: sect->data.elf_header.layout = 1; break;
	case ELFC_IDENT_CLASS_64: sect->data.elf_header.layout = 2; break;
	default: return 1;
	}

	read_layout(ELFC_BIN(elfc), off, &sect->data.elf_header.schema, sect->data.elf_header.layout, sect->data.elf_header.data);
...
int elfc_parse_program_header(elfc_t *elfc, u64 off, u8 class, u16 num, u16 size, elfc_sect_t *sect, alloc_t alloc)
{
	...
	switch (class) {
	case ELFC_IDENT_CLASS_32: sect->data.program_header.layout = 1; break;
	case ELFC_IDENT_CLASS_64: sect->data.program_header.layout = 2; break;
	default: return 1;
	}
	...
	for (int i = 0; i < num; i++) {
		...
		read_layout(ELFC_BIN(elfc), &o, &sect->data.program_header.tbl.schema, sect->data.program_header.layout, data);
	}
...
static uint elfc_image_section_flags(u64 flags)
{
	uint image_flags = 0;

	if (flags & (1 << SECTION_HEADER_FLAG_EXECINSTR)) {
		image_flags |= REVERSE_IMAGE_SECTION_EXEC;
	}
	if (flags & (1 << SECTION_HEADER_FLAG_WRITE)) {
		image_flags |= REVERSE_IMAGE_SECTION_WRITE;
	}
	if (flags & (1 << SECTION_HEADER_FLAG_ALLOC)) {
		image_flags |= REVERSE_IMAGE_SECTION_ALLOC;
	}

	return image_flags;
}

static u8 elfc_image_data(u8 data)
{
	switch (data) {
	case ELFC_IDENT_DATA_LE: return REVERSE_IMAGE_DATA_LE;
	case ELFC_IDENT_DATA_BE: return REVERSE_IMAGE_DATA_BE;
	default: return REVERSE_IMAGE_DATA_UNKNOWN;
	}
}
...
int elfc_parse_section_header(elfc_t *elfc, u64 off, u8 class, u16 num, u16 size, u16 shstrndx, elfc_sect_t *sect, alloc_t alloc)
{
	...
	switch (class) {
	case ELFC_IDENT_CLASS_32: sect->data.section_header.layout = 1; break;
	case ELFC_IDENT_CLASS_64: sect->data.section_header.layout = 2; break;
	default: return 1;
	}
	...
	for (int i = 0; i < num; i++) {
		...
		read_layout(ELFC_BIN(elfc), &o, &sect->data.section_header.tbl.schema, sect->data.section_header.layout, data);
	}

	const u64 *offset = tbl_get_cell(&sect->data.section_header.tbl, shstrndx, SECTION_HEADER_OFFSET);

	const char *section_names_offset = &((char *)elfc->bytes.data)[*offset];

	tbl_map(&sect->data.section_header.tbl, SECTION_HEADER_NAME_OFF, SECTION_HEADER_NAME, elfc_map_name, (void *)section_names_offset);
...
int elfc_parse_note_section(elfc_t *elfc, u64 off, u64 size, elfc_sect_t *sect, alloc_t alloc)
{
	...
	tbl_init_rows(&sect->data.note.tbl, 2, alloc);
...
int elfc_parse_dynamic_section(elfc_t *elfc, size_t off, u64 size, u8 class, elfc_sect_t *sect, alloc_t alloc)
{
	...
	switch (class) {
	case ELFC_IDENT_CLASS_32: sect->data.dynamic.layout = 1; break;
	case ELFC_IDENT_CLASS_64: sect->data.dynamic.layout = 2; break;
	default: return 1;
	}
	...
	while (off < end) {
		...
		read_layout(ELFC_BIN(elfc), &off, &sect->data.dynamic.tbl.schema, sect->data.dynamic.layout, data);
	...
	const char *strtab_offset = &((char *)elfc->bytes.data)[offset];
	tbl_map(&sect->data.dynamic.tbl, DYNAMIC_SECTION_VAL, DYNAMIC_SECTION_VAL_STR, elfc_map_dynamic_name, (void *)strtab_offset);
...
int elfc_parse_dynsym_section(elfc_t *elfc, u64 off, u64 size, u8 class, u64 dynstr_off, elfc_sect_t *sect, alloc_t alloc)
{
	...
	switch (class) {
	case ELFC_IDENT_CLASS_32: sect->data.dynsym.layout = 1; break;
	case ELFC_IDENT_CLASS_64: sect->data.dynsym.layout = 2; break;
	default: return 1;
	}
	...
	const char *dynstr_offset = &((char *)elfc->bytes.data)[dynstr_off];
	tbl_map(&sect->data.dynsym.tbl, DYNSYM_SECTION_NAME_OFF, DYNSYM_SECTION_NAME, elfc_map_name, (void *)dynstr_offset);
...
{
	elfc_reladyn_name_map_t *reladyn = priv;
...
int elfc_parse_reladyn_section(elfc_t *elfc, u64 off, u64 size, u8 class, uint dynsym_id, elfc_sect_t *sect, alloc_t alloc)
{
	...
	uint layout;
	switch (class) {
	case ELFC_IDENT_CLASS_32: layout = 1; break;
	case ELFC_IDENT_CLASS_64: layout = 2; break;
	default: return 1;
	}
	...
	tbl_map(&sect->data.reladyn.tbl, RELADYN_SECTION_BIND, RELADYN_SECTION_NAME, elfc_map_reladyn_name, &priv);
...
static int elfc_add_image_section(reverse_image_t *image, uint index, strv_t name, u32 type, u64 flags, u64 addr, u64 offset, u64 size,
				  u32 link, u32 info, u64 align, u64 entry_size, u8 data, elfc_sect_t *sect)
{
	reverse_image_section_t section = {
		...
		.link	      = link,
		.info	      = info,
		.data	      = elfc_image_data(data),
		.flags	      = elfc_image_section_flags(flags),
		.priv	      = sect,
	};
...
{
	if (elfc == NULL || image == NULL) {
		return 1;
	}

	log_info("reverse", "elfc", NULL, "Read %zu bytes", (int)elfc->bytes.used);

	log_info("reverse", "elfc", NULL, "Parsing magic");
	u8 magic[] = {0x7F, 0x45, 0x4C, 0x46};
	if (bin_cmp(ELFC_BIN(elfc), 0, magic, sizeof(magic)) != 0) {
	...
	sect->addr = off;
	bin_get(ELFC_BIN(elfc), sizeof(magic), &off);
	sect->size = off - sect->addr;
	dst.off += dputf(dst, "Magic: 0x%02X%02X%02X%02X\n", magic[0], magic[1], magic[2], magic[3]);
	...
	sect->addr = off;
	elfc_parse_ident(elfc, &off, sect, alloc);
	sect->size = off - sect->addr;
	dst.off += dputf(dst, "[ELF IDENT]\n");
	dst.off += schema_print_data(&sect->data.elf_ident.schema, 0, sect->data.elf_ident.data, dst);
	dst.off += dputf(dst, "\n");
	...
	sect->addr = off;
	elfc_parse_header(elfc, &off, class, sect, alloc);
	sect->size = off - sect->addr;
	dst.off += dputf(dst, "[ELF header]\n");
	dst.off += schema_print_data(&sect->data.elf_header.schema, 0, sect->data.elf_header.data, dst);
	dst.off += dputf(dst, "\n");

	u16 phentsize = *(u16 *)schema_get_val(&sect->data.elf_header.schema, ELF_HEADER_PHENTSIZE, sect->data.elf_header.data);
	u16 phnum     = *(u16 *)schema_get_val(&sect->data.elf_header.schema, ELF_HEADER_PHNUM, sect->data.elf_header.data);
	u64 phoff     = *(u64 *)schema_get_val(&sect->data.elf_header.schema, ELF_HEADER_PHOFF, sect->data.elf_header.data);
	u16 shentsize = *(u16 *)schema_get_val(&sect->data.elf_header.schema, ELF_HEADER_SHENTSIZE, sect->data.elf_header.data);
	u16 shnum     = *(u16 *)schema_get_val(&sect->data.elf_header.schema, ELF_HEADER_SHNUM, sect->data.elf_header.data);
	u64 shoff     = *(u64 *)schema_get_val(&sect->data.elf_header.schema, ELF_HEADER_SHOFF, sect->data.elf_header.data);
	u16 shstrndx  = *(u16 *)schema_get_val(&sect->data.elf_header.schema, ELF_HEADER_SHSTRNDX, sect->data.elf_header.data);
	u16 machine   = *(u16 *)schema_get_val(&sect->data.elf_header.schema, ELF_HEADER_MACHINE, sect->data.elf_header.data);

	switch (machine) {
	case ELF_MACHINE_X86:
	case ELF_MACHINE_AMD_X86_64: image->machine = REVERSE_IMAGE_MACHINE_X86; break;
	default: image->machine = REVERSE_IMAGE_MACHINE_UNKNOWN; break;
	}
	...
	sect->addr = phoff;
	elfc_parse_program_header(elfc, phoff, class, phnum, phentsize, sect, alloc);
	sect->size = phentsize * phnum;
	dst.off += dputf(dst, "[Program header]\n");
	dst.off += tbl_print(&sect->data.program_header.tbl, dst);
	dst.off += dputf(dst, "\n");
	...
	sect->addr = shoff;
	elfc_parse_section_header(elfc, shoff, class, shnum, shentsize, shstrndx, sect, alloc);
	sect->size = shentsize * shnum;
	dst.off += dputf(dst, "[Section header]\n");
	dst.off += tbl_print(&sect->data.section_header.tbl, dst);
	dst.off += dputf(dst, "\n");
	...
	for (uint i = 0; i < section_header_cnt; i++) {
		...
		u64 addr	= *(u64 *)tbl_get_cell(&sect->data.section_header.tbl, i, SECTION_HEADER_ADDR);
		...
		u32 link	= *(u32 *)tbl_get_cell(&sect->data.section_header.tbl, i, SECTION_HEADER_LINK);
		u32 info	= *(u32 *)tbl_get_cell(&sect->data.section_header.tbl, i, SECTION_HEADER_INFO);
		u64 align	= *(u64 *)tbl_get_cell(&sect->data.section_header.tbl, i, SECTION_HEADER_ALIGN);
		u64 entry_size	= *(u64 *)tbl_get_cell(&sect->data.section_header.tbl, i, SECTION_HEADER_ENTSIZE);

		elfc_sect_t *image_sect = NULL;
			if (name.len > 0) {
				uint id;
		...
		if (elfc_add_image_section(
			    image, i, name, type, flags, addr, offset, size, link, info, align, entry_size, data, image_sect)) {
			return 1;
		}
	}
	...
	for (uint i = 0; i < section_header_cnt; i++) {
		...
		case SECTION_HEADER_TYPE_NOTE: {
			...
			strv_t label = strvbuf_get(&elfc->strs, sect->label);
			log_info("reverse", "elfc", NULL, "Parsing %.*s", label.len, label.data);
			elfc_parse_note_section(elfc, offset, sect->size, sect, alloc);
			dst.off += dputf(dst, "\n");
			dst.off += dputf(dst, "[%.*s]\n", label.len, label.data);
			...
			for (uint i = 0; i < sect->data.note.notes.cnt; i++) {
				...
				for (uint j = l->members; j < l->members + l->members_cnt; j++) {
					...
					case SCHEMA_TYPE_STR: {
						dst.off += strv_print(strvbuf_get(&elfc->strs, *(size_t *)val), dst);
						break;
					}
					default: dst.off += schema_print_val(&sect->data.note.tbl.schema, 0, m->field, val, dst); break;
					}
				...
				case NOTE_TYPE_GNU_BUILD_ID: {
					dst.off += dputf(dst, "Build ID: ");
					for (uint j = 0; j < sizeof(note->data.build_id.bytes); j++) {
						dst.off += dputf(dst, "%x", note->data.build_id.bytes[j]);
					}
					dst.off += dputf(dst, "\n");
					break;
				}
		...
		case SECTION_HEADER_TYPE_STRTAB: {
			...
			log_info("reverse", "elfc", NULL, "Parsing %.*s", label.len, label.data);
			dst.off += dputf(dst, "[%.*s]\n", label.len, label.data);
			elfc_parse_strtab_section(elfc, offset, sect->size, sect, &dst, alloc);
			dst.off += dputf(dst, "\n");
			break;
		}
		case SECTION_HEADER_TYPE_DYNAMIC: {
			...
			strv_t label = strvbuf_get(&elfc->strs, sect->label);
			log_info("reverse", "elfc", NULL, "Parsing %.*s", label.len, label.data);
			elfc_parse_dynamic_section(elfc, offset, sect->size, class, sect, alloc);
			dst.off += dputf(dst, "[%.*s]\n", label.len, label.data);
			dst.off += tbl_print(&sect->data.dynamic.tbl, dst);
			dst.off += dputf(dst, "\n");
			break;
		}
		case SECTION_HEADER_TYPE_DYNSYM: {
			...
			strv_t label = strvbuf_get(&elfc->strs, sect->label);
			log_info("reverse", "elfc", NULL, "Parsing %.*s", label.len, label.data);
			elfc_parse_dynsym_section(elfc, offset, sect->size, class, dynstr_off, sect, alloc);
			dst.off += dputf(dst, "[%.*s]\n", label.len, label.data);
			dst.off += tbl_print(&sect->data.dynsym.tbl, dst);
			dst.off += dputf(dst, "\n");
			break;
		}
		}
	...
	for (uint i = 0; i < section_header_cnt; i++) {
		...
		case SECTION_HEADER_TYPE_RELA: {
			...
			strv_t label = strvbuf_get(&elfc->strs, sect->label);
			log_info("reverse", "elfc", NULL, "Parsing %.*s", label.len, label.data);
			elfc_parse_reladyn_section(elfc, offset, sect->size, class, dynsym_id, sect, alloc);
			dst.off += dputf(dst, "[%.*s]\n", label.len, label.data);
			dst.off += tbl_print(&sect->data.reladyn.tbl, dst);
			dst.off += dputf(dst, "\n");
			break;
		}
		}
	...
	for (uint i = 0; i < section_header_cnt; i++) {
		...
		case SECTION_HEADER_TYPE_PROGBITS: {
			...
			if (flags & (1 << SECTION_HEADER_FLAG_EXECINSTR)) {
				...
				strv_t label = strvbuf_get(&elfc->strs, sect->label);
				log_info("reverse", "elfc", NULL, "Found executable section %.*s", label.len, label.data);
			}
...
static int format_elf_probe(const format_driver_t *drv, const bin_t *bin)
{
	(void)drv;

	if (bin == NULL || bin->buf.used < 4 || bin->buf.data == NULL) {
		return 0;
	}

	const byte *data = bin->buf.data;
	return data[0] == 0x7F && data[1] == 'E' && data[2] == 'L' && data[3] == 'F' ? 100 : 0;
}

static int format_elf_parse(const format_driver_t *drv, const bin_t *bin, reverse_image_t *image, dst_t dst, alloc_t alloc)
{
	(void)drv;
	...
	elfc_t *elfc = mem_alloc(sizeof(*elfc));
	if (elfc == NULL || elfc_init(elfc, alloc) == NULL) {
		mem_free(elfc, sizeof(*elfc));
		return 1;
	}

	image->priv = elfc;

	int ret = buf_resize(&elfc->bytes, bin->buf.used);
	if (ret == 0) {
		elfc->bytes.used = bin->buf.used;
		mem_copy(elfc->bytes.data, bin->buf.used, bin->buf.data, bin->buf.used);
		ret = elfc_parse(elfc, image, dst, alloc);
	}

	return ret;
}

static int format_elf_emit(const format_driver_t *drv, reverse_image_t *image, asmc_t *asmc, alloc_t alloc)
{
	(void)drv;
	(void)alloc;

	if (image == NULL || image->priv == NULL || asmc == NULL) {
		return 1;
	}
	...
	arr_foreach(&image->sections, i, section)
	{
		...
		if (!section->asmc_init || section->priv == NULL) {
			return 1;
		}

		elfc_sect_t *sect	= section->priv;
		sect->data.program.asmc = section->asmc;
		section->asmc_init	= 0;
	}

	return elfc_asmc(image->priv, asmc);
}

static void format_elf_free(const format_driver_t *drv, reverse_image_t *image)
{
	(void)drv;

	if (image == NULL || image->priv == NULL) {
		return;
	}

	elfc_t *elfc = image->priv;
	elfc_free(elfc);
	mem_free(elfc, sizeof(*elfc));
	image->priv = NULL;
}

static format_driver_t s_format_elf = {
	.name  = STRVT("elf"),
	.desc  = "ELF format parser",
	.probe = format_elf_probe,
	.parse = format_elf_parse,
	.emit  = format_elf_emit,
	.free  = format_elf_free,
};

FORMAT_DRIVER(format_elf, &s_format_elf);
...
int elfc_asmc_schema(asmc_t *asmc, const schema_t *schema, const strv_t *labels, void *data, uint layout, int multi, uint id)
{
	...
	for (uint i = l->members; i < l->members + l->members_cnt; i++) {
		...
		asmc_op_t *op		 = asmc_add_op(asmc, 0, ASMC_OP_LABEL);
		...
		case 1: {
			...
			op->dst.val = *(u8 *)schema_get_val(schema, i, data);
			break;
		}
		case 2: {
			op	    = asmc_add_op(asmc, 0, ASMC_OP_WORD);
			op->dst.val = *(u16 *)schema_get_val(schema, i, data);
			break;
		}
		case 4: {
			op	    = asmc_add_op(asmc, 0, ASMC_OP_LONG);
			op->dst.val = *(u32 *)schema_get_val(schema, i, data);
			break;
		}
		case 8: {
			op	    = asmc_add_op(asmc, 0, ASMC_OP_QUAD);
			op->dst.val = *(u64 *)schema_get_val(schema, i, data);
			break;
		}
		default: {
	...
	if (*unknown_zeros > 0) {
		if (*unknown_zeros > 3) {
			...
			for (uint i = 0; i < *unknown_zeros; i++) {
				...
				asmc_op_t *op = asmc_add_op(asmc, 0, ASMC_OP_BYTE);
				op->dst.val   = 0;
			}
		}
		*unknown_zeros = 0;
...
int elfc_asmc(const elfc_t *elfc, asmc_t *asmc)
{
	...
	u64 addr = 0;
	while (addr < elfc->bytes.used) {
		elfc_sect_t *sect = elfc_find_sect(elfc, addr);
		if (sect == NULL) {
				if (label == 0) {
				...
				byte val = *(byte *)buf_get(&elfc->bytes, addr);
				if (val == 0) {
					unknown_zeros++;
				} else {
			...
			case ELF_SECT_TYPE_BYTES: {
				...
				for (uint off = 0; off < sect->size; off++) {
					op	    = asmc_add_op(asmc, 0, ASMC_OP_BYTE);
					op->dst.val = *(u8 *)buf_get(&elfc->bytes, sect->addr + off);
				}
				break;
			}
			case ELF_SECT_TYPE_MAGIC: {
				...
				strvbuf_add(&asmc->strs, strvbuf_get(&elfc->strs, sect->label), &op->str);
				op	    = asmc_add_op(asmc, 0, ASMC_OP_LONG);
				op->dst.val = *(u32 *)buf_get(&elfc->bytes, sect->addr);
				break;
			}
			...
			case ELF_SECT_TYPE_NOTE: {
				...
				for (uint i = 0; i < sect->data.note.notes.cnt; i++) {
					...
					case NOTE_TYPE_GNU_ABI_TAG: {
						...
						case NOTE_SECTION_ABI_TAG_OS_LINUX: {
							op	    = asmc_add_op(asmc, 0, ASMC_OP_LONG);
							op->dst.val = NOTE_SECTION_ABI_TAG_ELF_NOTE_OS_LINUX;
						}
						default: break;
						}
					...
					case NOTE_TYPE_GNU_BUILD_ID: {
						...
						for (uint j = 0; j < sizeof(note->data.build_id.bytes); j++) {
							op	    = asmc_add_op(asmc, 0, ASMC_OP_BYTE);
							op->dst.val = note->data.build_id.bytes[j];
						}
						break;
					}
					case NOTE_TYPE_GNU_PROPERTIES: {
						...
						arr_foreach(&note->data.gnu_properties.arr, j, gnu_property)
						{
							...
							case NOTE_SECTION_GNU_PROPERTY_FEATURES: {
								...
								op	    = asmc_add_op(asmc, 0, ASMC_OP_LONG);
								op->dst.val = NOTE_SECTION_GNU_PROPERTY_X86_FEATURE_1_AND;
								...
								op	    = asmc_add_op(asmc, 0, ASMC_OP_LONG);
								op->dst.val = (gnu_property->data.features.ibt
							...
							case NOTE_SECTION_GNU_PROPERTY_ISA: {
								...
								op	    = asmc_add_op(asmc, 0, ASMC_OP_LONG);
								op->dst.val = NOTE_SECTION_GNU_PROPERTY_X86_ISA_1_NEEDED;
								...
								case NOTE_SECTION_GNU_PROPERTY_ISA_BASELINE: {
									op	    = asmc_add_op(asmc, 0, ASMC_OP_LONG);
									op->dst.val = NOTE_SECTION_GNU_PROPERTY_X86_ISA_1_BASELINE;
									break;
								}
								default: break;
								}
			...
			case ELF_SECT_TYPE_PROGRAM: {
				...
				{
					asmc_op_t *d = asmc_add_op(asmc, op->addr, op->type);
					d->dst	     = op->dst;
					d->src	     = op->src;
					d->sib	     = op->sib;
